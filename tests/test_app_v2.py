...
class AppV2Tests(unittest.TestCase):
    def setUp(self):
        self.original_open_bus = app_v2.open_bus

    def tearDown(self):
        app_v2.open_bus = self.original_open_bus

    def _fake_bus(self, servos):
        port_handler = FakePortHandler()
        app_v2.open_bus = lambda port, baudrate: (port_handler, FakePacketHandler(servos))
        return port_handler
    ...
    def test_run_set_zero_stores_pose_and_closes_port(self):
        ...
        results, error = app_v2.run_set_zero("/dev/fake", 1000000, [1, 2, 3], log=logs.append)
        ...
        ok, message = app_v2.summarize(results, error)
        self.assertTrue(ok)
    ...
    def test_run_set_zero_reports_when_no_servo_answers(self):
        ...
        results, error = app_v2.run_set_zero("/dev/fake", 1000000, [1, 2], log=lambda *_: None)

        self.assertEqual(results, {})
        self.assertIn("Cap motor ha respost", error)
        self.assertTrue(port_handler.closed)
        self.assertEqual(app_v2.summarize(results, error), (False, error))
    ...
    def test_run_set_zero_reports_port_errors(self):
        ...
        app_v2.open_bus = failing_open_bus

        results, error = app_v2.run_set_zero("/dev/fake", 1000000, [1], log=lambda *_: None)
    ...
    def test_summarize_flags_servos_that_did_not_take_the_zero(self):
        ...
        ok, message = app_v2.summarize(results, None)
